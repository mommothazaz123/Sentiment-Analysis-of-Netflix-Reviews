...
from data.dataset import get_test_data, get_training_data
from models.train_model import TrainModel

tf.app.flags.DEFINE_string('train_path', os.path.abspath(
    os.path.join(os.path.dirname("__file__"), '..', 'data/tf_records/training_file_0.tfrecord')),
                           'Path for the training data.')
tf.app.flags.DEFINE_string('test_path', os.path.abspath(
    os.path.join(os.path.dirname("__file__"), '..', 'data/tf_records/test_file_0.tfrecord')),
                           'Path for the test data.')

tf.app.flags.DEFINE_string('word2idx', os.path.abspath(
    os.path.join(os.path.dirname("__file__"), '..', 'data/preprocessed/dnd_word2idx.txt')),
...
tf.app.flags.DEFINE_integer('n_train_samples', 552 + 524,
                            'Number of all training sentences.'
                            )

tf.app.flags.DEFINE_integer('n_test_samples', 126 + 143,
                            'Number of all training sentences.'
                            )
...
def main(_):
    ...
    with training_graph.as_default():
        ...
        logits_test, probs_test = train_model.compute_prediction(x_test, seq_length_test, dropout_keep_prob,
                                                                 reuse_scope=True)
    ...
    with tf.Session(graph=training_graph) as sess:
        ...
        for epoch in range(FLAGS.num_epoch):
            ...
            print('epoch_nr: %i, train_loss: %.3f, train_acc: %.3f, test_acc: %.3f' % (
            epoch, loss_avg, acc_avg_train, acc_avg_test))
